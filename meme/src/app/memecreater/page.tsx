...
const Meme = ({searchParams} : {searchParams : SingleMeme}) => {
  ...
  const memeForm = async(event:React.SyntheticEvent)=>{
    ...
    if (!text1.current?.value || !text2.current?.value || !text3.current?.value || !text4.current?.value || !text5.current?.value) {
      alert('Please enter all the text fields');
      return;
    }
    setSubmit('loading...')
    
    const response = await fetch(`https://api.imgflip.com/caption_image?template_id=${searchParams.id}&username=BilalMIqbal &password=memex100@/&text0=${text1.current?.value}&text1=${text2.current?.value}&text2=${text3.current?.value}&text3=${text4.current?.value}&text4=${text5.current?.value}`,{
       method:'POST'
      })
